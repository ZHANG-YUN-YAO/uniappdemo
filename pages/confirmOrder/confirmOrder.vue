...
<script>
	...
	export default {
		...
		onShow() {
			if(this.seledefaultaddress!=''){
				if(this.seledefaultaddress.province_name){
					this.defaultaddress = this.seledefaultaddress.province_name
				}
				if(this.seledefaultaddress.district_name){
					this.defaultaddress = this.seledefaultaddress.province_name+this.seledefaultaddress.district_name
				}
				if(this.seledefaultaddress.city_name){
					this.defaultaddress = this.seledefaultaddress.province_name+this.seledefaultaddress.district_name+this.seledefaultaddress.city_name
				}
			}else{
		...
		methods:{
			...
			toseleaddress(){
				this.ifdisplay = true;
				// uni.navigateTo({
				// 	url:'../toseleAddress/toseleAddress?addressid='+this.getaddress_id
				// })
			},
			getdefaultaddress(){
				uni.request({
				    url: this.global_api+'/api/address/get_def',
				    data: {
				    },
				    method:'POST',
				    header: {
				    ...
				    success: (res) => {
							if(res.data.status==200){		
								...
								if(res.data.result.district){
									this.defaultaddress = res.data.result.province+res.data.result.district
								}
								if(res.data.result.city){							
									this.defaultaddress = res.data.result.province+res.data.result.district+res.data.result.city
								}
							}else{
								uni.showToast({
									icon: 'none',
